...
class NICAsiaBank(Reconcile):
    def __init__(self,bank_name,excel_writer):
        ...
        try:
            ...
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NICAsia_SOA_FILE, encoding='latin-1', skiprows=1)
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            ...
            if "WT10000" in str(row['Desc2']):
                ...
                ind = str(row['Desc2']).index('NPS-IF-')
                id = str(row['Desc2'])[ind+7:ind+14]
                self.bank_statement_df.at[index, 'Transaction Id'] = id
                continue

            elif "FTMS-" in str(row['Desc2']):
                ind = str(row['Desc2']).index('FTMS-')
                id = str(row['Desc2'])[ind+5:ind+11]
                self.bank_statement_df.at[index, 'Transaction Id'] = id
                continue

            elif "10000" in str(row['Desc2']):
                ind = str(row['Desc2']).index('10000')
                id = str(row['Desc2'])[ind+5:ind+12]
                self.bank_statement_df.at[index, 'Transaction Id'] = id
                continue

            elif "10000" in str(row['Desc1']):
                ind = str(row['Desc1']).index('10000')
                id = str(row['Desc1'])[ind+5:ind+12]
                self.bank_statement_df.at[index, 'Transaction Id'] = id
                self.bank_statement_df.at[index, 'Transaction Id'] = id
