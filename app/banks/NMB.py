...
class NMBBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        try:
            ...
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NMB_SOA_FILE, encoding='latin-1', skiprows=1)
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            ...
            #         self.bank_statement_df.loc[index, 'Transaction Id'] = id
                ...
                ind = str(row['Merged_Description']).index('FTMS-')
                id = str(row['Merged_Description'])[ind+5:ind+11]
                self.bank_statement_df.loc[index, 'Transaction Id'] = id
                continue
            elif "10000" in str(row["Merged_Description"]):
                ind = str(row['Merged_Description']).index('10000')
                id = str(row['Merged_Description'])[ind+5:ind+12]
