...
class LaxmiBank(Reconcile):    
    ...
    def get_phase4_df(self):
        try:
            ...
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.Laxmi_SOA_FILE, encoding='latin-1')
            self.bank_statement_df.drop('Number', axis=1, inplace=True)
            try:
                display(self.soa_statement_df.loc[0, 'Transaction Id'])
            except:
                self.soa_statement_df = pd.read_csv(SOA_FILEPATH.Laxmi_SOA_FILE, encoding='latin-1', skiprows=1)
                self.bank_statement_df.drop('Number', axis=1, inplace=True)
        except:
            self.proceed = False
    ...
    def preprocessing_bank_stmt_phase_4(self):
        ...
        self.bank_statement_df = self.bank_statement_df.loc[self.bank_statement_df['Ac.Number'] == 14242431110001, :]
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            if "NPS-IF-" in str(row['Desc1']):
                ind = str(row['Desc1']).index('NPS-IF-')
                # id = id[0]
                id = str(row['Desc1'])[ind+7:ind+14]
                ...
                id = id[index_10000+5:index_10000+11]
