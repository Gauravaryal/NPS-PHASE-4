...
class EverestBank(Reconcile):
    ...
    def get_phase4_df(self):
        try:
            ...
            try:
                ...
                self.soa_statement_df = pd.read_csv(SOA_FILEPATH.EVEREST_SOA_FILE, encoding='latin-1', skiprows=1)
        except:
            self.proceed = False
    ...
    def main(self):
        ...
        '''
        display(f'{self.bank_statement_df.columns}')
        ...
        display(f'1 {self.bank_statement_df.columns}')
        self.extracting_tid_from_bank_stmt_phase_4()
        display(f'12 {self.bank_statement_df.columns}')
        self.preprocessing_soa_stmt()
        display(f'23 {self.bank_statement_df.columns}')
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            if "D100000000" in str(row['Desc2']):
                ...
                self.bank_statement_df.loc[index, 'Transaction Id'] = id
                continue
            elif "S100000" in str(row['Desc2']):
                index = str(row['Desc2']).index('S100000')
                id =  str(row['Desc2'])[index+7:index+14]
                self.bank_statement_df.loc[index, 'Transaction Id'] = id
                continue
