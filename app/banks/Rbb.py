...
class RBBBank(Reconcile):
    ...
    def get_phase4_df(self):
        ...
        try:
            ...
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.RBB_SOA_FILE, encoding='latin-1', skiprows=1)
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            if "NPS-IF-" in str(row["Transaction Detail"]):
                ind = str(row['Transaction Detail']).index("NPS-IF-")
            ...
            #         self.bank_statement_df.loc[index, 'Transaction Id'] = id
                ...
                ind = str(row['Transaction Detail']).index('10000')
                id = str(row['Transaction Detail'])[ind+5:ind+12]
                try:
                    num = int(id)
                except:
                    cut = str(row['Transaction Detail'])[ind+5:]
                    ind = cut.index('10000')
                    id = cut[ind+5:ind+12]
                self.bank_statement_df.loc[index, 'Transaction Id'] = id
            else:
