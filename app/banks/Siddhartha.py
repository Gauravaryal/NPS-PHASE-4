...
class SiddharthaBank(Reconcile):
    ...
    def get_phase4_df(self):
        ...
        self.soa_statement_df = pd.read_csv(SOA_FILEPATH.SIDDHARTHA_SOA_FILE, encoding='utf-8')
        try:
            display(self.soa_statement_df.loc[0, 'Transaction Id'])
        except:
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.SIDDHARTHA_SOA_FILE, encoding='latin-1', skiprows=1)
