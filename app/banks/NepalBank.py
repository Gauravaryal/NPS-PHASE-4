...
class NepalBank(Reconcile):
    def __init__(self, bank_name, excelwriter):
        ...
        try:
            ...
            self.soa_statement_df = pd.read_csv(SOA_FILEPATH.NEPALBANK_SOA_FILE, encoding='latin-1', skiprows=1)
    ...
    def extracting_tid_from_bank_stmt_phase_4(self):
        ...
        for index, row in self.bank_statement_df.iterrows():
            for roww in (row['Desc3'] , row['Desc2']):
                if "NPS-IF-" in roww:
                    ind = str(roww).index('NPS-IF-')
                    id = str(roww)[ind+7:ind+14]
                    self.bank_statement_df.loc[index, 'Transaction Id'] = id
                    break
                
                elif "FTMS-" in roww:
                    ind = str(roww).index('FTMS-')
                    id = str(roww)[ind+5:ind+11]
                    self.bank_statement_df.loc[index, 'Transaction Id'] = id
                    break
                
                elif "10000" in roww:
                    ind = str(roww).index('10000')
                    id = str(roww)[ind+5:ind+12]
                    self.bank_statement_df.loc[index, 'Transaction Id'] = id
                    break
